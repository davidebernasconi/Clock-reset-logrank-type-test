### 
### MARKOV DATA (no effetto waiting time in CF scale)
### 
...
for (i in 1:nsim) {
  ...
  U<-runif(n, min=0, max=1)
  t.htx.death<- (t.htx ^ q - (log(U)/(k^q))) ^ (1/q) - t.htx
...
#------------------------------------------------------#
####  simulations under some alternative H1, n=100  ####
#------------------------------------------------------#
...
k<-0.6
q<-0.7
b<-0 # no effetto waiting time in CF scale
haz.list.death<-q*k*((k*t)^(q-1))
surv.list.death<-exp(-((k*t)^q))

b.ill<-(-log(3))/q # if treatment has a favorable effect, power CB test > manetl byar
...
for (i in 1:nsim) {
  ...
  # generate latent times from transplant to death
  U<-runif(n, min=0, max=1)
  t.htx.death<- (t.htx ^ q - (log(U)/((k*exp(b.ill))^q))) ^ (1/q) - t.htx
...
#------------------------------------------------------#
####  simulations under some alternative H1, n=200  ####
#------------------------------------------------------#
