...
for (i in 1:nsim) {
  ...
  htdatasplitHTx$Tstop<-ifelse(htdatasplitHTx$Tstop==htdatasplitHTx$Tstart,htdatasplitHTx$Tstop+0.001,htdatasplitHTx$Tstop)
...
for (i in 1:nsim) {
  ...
  # generate latent times from transplant to death
  U<-runif(n, min=0, max=1)
  t.htx.death<- ( (((-log(U))*(b*t.htx+1))^(1/q))/((5^(-b*t.htx))*k*exp(b.ill)) )^(1/(b*t.htx+1))
  t.htx.death<-ifelse(is.na(t.htx.death),0,t.htx.death) 
  # correzione necessaria perchè alcuni tempi molti piccoli (vicini a 0) vengono settati a NA...
